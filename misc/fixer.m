if 1 
    ...
    if isa(dag.layers(ii).block, 'dagnn.Reshape')
      switch dag.layers(ii).name
        ...
        case 'rpn_cls_prob_reshape'
          dag.layers(ii).block.shape = [-1 0 24] ; % 18 for pascal
        otherwise
          fprintf('%s\n', dag.layers(ii).name) ;
      end
    end
  ...
  modPath = '/users/albanie/coding/libs/matconvnets/contrib-matconvnet/data/models-import/faster-rcnn-vggvd-coco.mat' ;
