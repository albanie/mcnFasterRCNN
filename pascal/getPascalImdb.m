...
%GETPASCALIMDB loads Pascal VOC image database
%  IMDB = GETPASCALIMDB(OPTS) builds an image database for training and 
%  testing on the pascal dataset
%
...
% ------------------------------------------------
  imdb.images.name = horzcat(imdb07.images.name, imdb12.images.name) ;
  imdb.images.set = horzcat(imdb07.images.set, imdb12.images.set) ;
...
% ------------------------------------------------
  ...
  for ii = 1:numel(imdb.images.name)
    ...
    msg = 'pascal class labels do not lie in the expected range' ;
    assert(all(2 <= gt.classes) && all(gt.classes <= 21), msg) ;
    annotations{ii} = gt ;
