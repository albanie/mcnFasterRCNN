function [aps, speed] = faster_rcnn_evaluate(varargin)
%FASTER_RCNN_EVALUATE  Evaluate a trained Faster-RCNN model on 
% PASCAL VOC 2007

% Based on the `fast_rcnn_evaluate.m` script by Hakan Bilen.
...
addpath(fullfile(vl_rootnn, 'data/datasets/VOCdevkit/VOCcode')) ;
opts.expDir = fullfile(opts.dataDir, 'faster-rcnn-vgg16-pascal07') ;
[opts, varargin] = vl_argparse(opts, varargin) ;

opts.imdbPath = fullfile(opts.expDir, 'imdb.mat');
opts.modelPath = fullfile(opts.dataDir, 'model-imports', ...
                             'faster-rcnn-vggvd-pascal.mat') ;
